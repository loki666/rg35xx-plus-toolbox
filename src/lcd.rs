...
mod ioctl {
    ...
    ioctl_write_ptr_bad!(set_brightness, DISP_LCD_SET_BRIGHTNESS, [usize]); 
    ioctl_write_ptr_bad!(get_brightness, DISP_LCD_GET_BRIGHTNESS, [usize]);   
}
...
pub fn set_brightness(fd: RawFd, percent: u32) -> io::Result<()> {
    let raw_value: usize = (percent * (BRIGHTNESS_MAX_LEVEL - BRIGHTNESS_MIN_LEVEL) / 100 + BRIGHTNESS_MIN_LEVEL).try_into().unwrap();
    let data: [usize; 4] = [0, raw_value, 0, 0];
